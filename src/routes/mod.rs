//! HTTP route handlers for the bot-camp server.

mod auth;
mod broken_html;
mod canonical;
mod challenge;
mod content;
mod dashboard;
mod delay;
mod discovery;
mod encoding;
mod headers;
mod health;
mod honeypot;
mod js_render;
mod large_response;
mod normalize;
mod ratelimit;
mod redirect;
mod response;
mod robots;
mod status;

pub use auth::basic;
pub use broken_html::broken_html;
pub use canonical::canonical;
pub(crate) use challenge::enforce as challenge_enforce;
pub use challenge::probe as challenge_probe;
pub use challenge::set_config as challenge_set_config;
pub use content::content;
pub use dashboard::assets as dashboard_assets;
pub use dashboard::index as dashboard_index;
pub use dashboard::snapshot as dashboard_snapshot;
pub use dashboard::ws as dashboard_ws;
pub use delay::delay;
pub use discovery::discovery;
pub use discovery::target as discovery_target;
pub use encoding::encoding;
pub use headers::echo;
pub use headers::set;
pub use health::health;
pub(crate) use honeypot::enforce as honeypot_enforce;
pub use honeypot::reset as honeypot_reset;
pub use honeypot::set_config as honeypot_set_config;
pub use honeypot::status as honeypot_status;
pub use honeypot::trap as honeypot_trap;
pub use js_render::js_render;
pub use large_response::large_response;
pub use normalize::normalize;
pub(crate) use ratelimit::enforce as ratelimit_enforce;
pub use ratelimit::probe as ratelimit_probe;
pub use ratelimit::reset as ratelimit_reset;
pub use ratelimit::set_config as ratelimit_set_config;
pub use ratelimit::status as ratelimit_status;
pub use redirect::redirect;
pub use redirect::redirect_chain;
pub use redirect::redirect_loop;
pub use redirect::redirect_meta_refresh;
pub use redirect::redirect_refresh;
pub use response::response;
pub use robots::robots_meta;
pub use robots::robots_txt;
pub use robots::set_robots_txt;
pub use status::status;
